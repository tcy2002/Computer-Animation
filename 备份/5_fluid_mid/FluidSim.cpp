...
void FluidSim::advectValues() {
	...
	for (int y = 1; y < m_res_y - 1; ++y) {
		for (int x = 1; x < m_res_x - 1; ++x) {
			...
			double last_x = mid_x - m_dt * last_x_velocity / m_dx;
			double last_y = mid_y - m_dt * last_y_velocity / m_dx;
	...
	for (int y = 1; y < m_res_y - 1; ++y) {
		for (int x = 1; x < m_res_x; ++x) {
			...
			// outside walls
			if (last_x <= 1.5) last_x = 1.5 + m_acc;
			if (last_y <= 1) last_y = 1 + m_acc;
			if (last_x >= m_res_x - 1.5) last_x = m_res_x - 1.5 - m_acc;
			if (last_y >= m_res_y - 2) last_y = m_res_y - 2 - m_acc;
	...
	for (int y = 1; y < m_res_y; ++y) {
		for (int x = 1; x < m_res_x - 1; ++x) {
			...
			// outside walls
			if (last_x <= 1) last_x = 1 + m_acc;
			if (last_y <= 1.5) last_y = 1.5 + m_acc;
			if (last_x >= m_res_x - 2) last_x = m_res_x - 2 - m_acc;
			if (last_y >= m_res_y - 1.5) last_y = m_res_y - 1.5 - m_acc;
